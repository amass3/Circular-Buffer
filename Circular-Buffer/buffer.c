...
SYSCALL_DEFINE0(init_buffer_421){
	...
	if((head == NULL) && (tail == NULL)){
		node_421_t* temp = (struct node_421*)kmalloc(sizeof(node_421_t), GFP_KERNEL);
		...
		for(i = 0; i < SIZE_OF_BUFFER; i++){
			...
			if(i == 0){
				...
				temp->next = (struct node_421*)kmalloc(sizeof(node_421_t), GFP_KERNEL);
...
SYSCALL_DEFINE1(insert_buffer_421, int, i){
	...
	if((head != NULL) && (tail != NULL)){
		...
		//Inserting fails if the buffer is already full.
			...
			printk("the buffer is already full.\n");
			return -1;
		}
	}else{

		printk("cannot insert data into an uninitialized buffer.\n");
		return -1;
	}
